...
int notasManipulation(int *Alunos_quant, float *NOTAS, float *MEDIA, float *Maior_nota, float *Menor_nota, int *Acima_media, float *MEDIANA, int *Moda_pos, int Novos_alunos_bool, int *Maior_nota_pos, int *Menor_nota_pos, int Novos_alunos, float *Notas_org){
    ...
    if(Novos_alunos_bool == 0){
        ...
        *MEDIA = *MEDIA/(*Alunos_quant);
        for(int i = 0; i<*Alunos_quant; i++){
            if(NOTAS[i]>*MEDIA){
                *Acima_media+=1;
            }
            for(int j = 1+i; j<*Alunos_quant; j++){
                if((NOTAS[j]>NOTAS[i])&&(NOTAS[j]>NOTAS[(int)*Maior_nota])){
                    *Maior_nota = j;
                }
                if(NOTAS[j]<NOTAS[i]&&(NOTAS[j]<NOTAS[(int)*Menor_nota])){
                    *Menor_nota = j;
                }
            }
        }
...
int main(void){
    ...
    float *NOTAS = calloc(Alunos_quant, sizeof(*NOTAS)), *Notas_org = calloc(Alunos_quant, sizeof(*Notas_org)),MEDIA = 0.0, Maior_nota = 0, Menor_nota = 0, MEDIANA;
    ...
    float *MEDIA_pont = &MEDIA, *Maior_nota_pont = &Maior_nota, *Menor_nota_pont = &Menor_nota, *MEDIANA_pont = &MEDIANA;

    if(NOTAS != NULL){


        notasManipulation(Alunos_quant_pont, NOTAS, MEDIA_pont, Maior_nota_pont, Menor_nota_pont, Acima_media_pont, MEDIANA_pont, Moda_pos_pont, Novos_alunos_bool, Maior_nota_pos_pont, Menor_nota_pos_pont, Novos_alunos, Notas_org);
        ...
        Novos_alunos_bool = 1;
        NOTAS = realloc(NOTAS, Alunos_quant);
        Notas_org = realloc(Notas_org, Alunos_quant);
